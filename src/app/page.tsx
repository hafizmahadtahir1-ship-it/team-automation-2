...
interface OrbConfig { x: string; y: string; s: number; c: string; op: number; an: string; }
interface CounterProps { to: number; prefix?: string; suffix?: string; label: string; }
interface SRProps { children: ReactNode; delay?: number; dir?: "up"|"left"|"right"|"scale"; }
interface TiltCardProps { children: ReactNode; style?: CSSProperties; intensity?: number; }
interface MagBtnProps { children: ReactNode; onClick?: () => void; gold?: boolean; outline?: boolean; style?: CSSProperties; }
interface FeatureItem { icon: string; title: string; desc: string; delay: number; }
interface PricingItem { name: string; price: string; period: string; sub: string; features: string[]; featured: boolean; cta: string; href: string; }
interface TestiItem { q: string; name: string; role: string; bg: string; }
interface SectionHeadProps { eyebrow: string; title: string; italic?: string; sub?: string; center?: boolean; }

/* ══════════════════════════════════════════
   PARTICLE CANVAS
══════════════════════════════════════════ */
function CosmicCanvas({ mousePos }: { mousePos: MousePos }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const mp = useRef<MousePos>({ x: -9999, y: -9999 });
...
/* ══════════════════════════════════════════
   3D TILT CARD
══════════════════════════════════════════ */
function TiltCard({ children, style, intensity = 14 }: TiltCardProps) {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (ref.current) {
        ...
        const s = a ? 1.025 : 1;
        ref.current.style.transform = `perspective(900px) rotateX(${cur.current.rx}deg) rotateY(${cur.current.ry}deg) scale3d(${s},${s},1)`;
      }
...
/* ══════════════════════════════════════════
   MAGNETIC BUTTON
══════════════════════════════════════════ */
function MagBtn({ children, onClick, gold, outline, style }: MagBtnProps) {
...
/* ══════════════════════════════════════════
   SCROLL REVEAL
══════════════════════════════════════════ */
function SR({ children, delay = 0, dir = "up" }: SRProps) {
  const ref = useRef<HTMLDivElement>(null);
  ...
  const from: Record<string, string> = { up:"translateY(55px)", left:"translateX(-55px)", right:"translateX(55px)", scale:"scale(0.9)" };
  return (
    <div ref={ref} style={{
...
function Counter({ to, prefix = "", suffix = "", label }: CounterProps) {
  ...
  return (
    <div ref={ref} style={{ textAlign: "center" }}>
      ...
      <div style={{ color:"rgba(255,255,255,0.28)", fontSize:"11px", letterSpacing:"0.12em", textTransform:"uppercase", marginTop:"10px" }}>{label}</div>
    </div>
  );
}
...
function SlackCard() {
  ...
  return (
    <TiltCard intensity={13} style={{
      ...
      border:"1px solid rgba(212,175,55,0.22)", borderRadius:"28px", padding:"26px 28px", width:"340px",
      ...
      {state === "rejected" && (
        <div style={{ textAlign:"center", padding:"14px", borderRadius:"13px", background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.22)", animation:"popIn 0.45s cubic-bezier(0.16,1,0.3,1)" }}>
          ...
          <div style={{ color:"rgba(255,255,255,0.25)", fontSize:"11px", marginTop:"3px" }}>@sarah_ops notified instantly</div>
          <button onClick={() => setState("idle")} style={{ marginTop:"9px", background:"none", border:"none", color:"rgba(255,255,255,0.16)", fontSize:"10px", cursor:"pointer" }}>↩ reset</button>
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:"10px", flexWrap:"wrap", marginBottom:"14px" }}>
        <input type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()}
          ...
          style={{ flex:"1 1 200px", maxWidth:"280px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.12)",
            borderRadius:"100px", padding:"15px 24px", fontSize:"15px", color:"#fff", outline:"none", fontFamily:"'DM Sans',sans-serif", transition:"border 0.3s, box-shadow 0.3s" }}
...
/* ══════════════════════════════════════════
   SECTION HEADER
══════════════════════════════════════════ */
function SectionHead({ eyebrow, title, italic, sub, center }: SectionHeadProps) {
  return (
    <SR>
      <div style={{ textAlign: center ? "center" : "left", marginBottom:"52px" }}>
        <div style={{ color:"rgba(212,175,55,0.55)", fontSize:"10px", letterSpacing:"0.28em", textTransform:"uppercase", marginBottom:"14px" }}>{eyebrow}</div>
        ...
        {sub && <p style={{ color:"rgba(255,255,255,0.35)", fontSize:"17px", lineHeight:1.8, fontWeight:300, maxWidth:"520px", margin: center ? "12px auto 0" : "12px 0 0" }}>{sub}</p>}
      </div>
    </SR>
  );
}
...
/* ══════════════════════════════════════════
   FAQ ITEM — separate component to fix hooks-in-loop
══════════════════════════════════════════ */
  ...
  return (
    <div style={{ borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
      <button onClick={() => setOpen(!open)} style={{ width:"100%", background:"none", border:"none", cursor:"pointer",
        ...
        {q}
        <span style={{ fontSize:"20px", color:"rgba(212,175,55,0.55)", transition:"transform 0.35s cubic-bezier(0.16,1,0.3,1)", transform: open ? "rotate(45deg)" : "rotate(0deg)", flexShrink:0, marginLeft:"16px" }}>+</span>
      </button>
...
export default function LandingPage() {
  ...
  const features: FeatureItem[] = [
    ...
    { icon:"📋", title:"Full Audit Trail", desc:"Every action logged: timestamp, user, context. Always compliant, always exportable to CSV.", delay:140 },
    ...
    <div style={{ background:"#04040a", minHeight:"100vh", color:"#fff", overflowX:"hidden", fontFamily:"'DM Sans',sans-serif", position:"relative" }}>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes textGlow{0%,100%{text-shadow:0 0 40px rgba(212,175,55,0.18)}50%{text-shadow:0 0 60px rgba(212,175,55,0.42),0 0 120px rgba(212,175,55,0.14)}}
        ...
        @keyframes divShimmer{0%{opacity:0.3}50%{opacity:1}100%{opacity:0.3}}
        .chov{transition:border-color 0.3s,transform 0.25s,box-shadow 0.25s;}
        .chov:hover{border-color:rgba(212,175,55,0.28)!important;transform:translateY(-4px)!important;box-shadow:0 24px 60px rgba(0,0,0,0.45)!important;}
        .ibox{width:48px;height:48px;border-radius:13px;display:flex;align-items:center;justify-content:center;font-size:19px;background:rgba(212,175,55,0.08);border:1px solid rgba(212,175,55,0.17);margin-bottom:18px;transition:all 0.3s;}
        .chov:hover .ibox{background:linear-gradient(135deg,#D4AF37,#8B6914)!important;border-color:transparent!important;box-shadow:0 8px 28px rgba(212,175,55,0.35)!important;}
        @media(max-width:900px){
          .hgrid{flex-direction:column!important;}
          .hrw{display:none!important;}
          .tcol{grid-template-columns:1fr 1fr!important;}
          .prow{flex-direction:column!important;align-items:center!important;}
          .prow>*{width:100%!important;max-width:420px!important;}
          .sgrid{grid-template-columns:repeat(2,1fr)!important;}
          nav{padding:0 20px!important;}
          .nlnks{display:none!important;}
        }
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:300, height:"64px",
        ...
        backdropFilter:"blur(28px)", borderBottom:"1px solid rgba(255,255,255,0.04)", transition:"background 0.4s" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          ...
          <span style={{ fontFamily:"'Syne',sans-serif", fontSize:"17px", fontWeight:800 }}>Team<span style={{ color:"#D4AF37" }}>Automation</span></span>
        </div>
        <div className="nlnks" style={{ display:"flex", alignItems:"center", gap:"28px" }}>
          {(["Features","How it works","Pricing","FAQ"] as const).map((l, i) => {
            const ids = ["features","how","pricing","faq"];
            return (
              <button key={l} onClick={() => go(ids[i])} style={{ background:"none", border:"none", cursor:"pointer",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          ...
          <MagBtn gold onClick={() => go("cta")} style={{ padding:"9px 20px", borderRadius:"100px", fontSize:"13px", fontWeight:700, color:"#0a0a0a", fontFamily:"'Syne',sans-serif", letterSpacing:"0.04em", boxShadow:"0 4px 18px rgba(212,175,55,0.3)" }}>Get Early Access</MagBtn>
        </div>
      </nav>
      ...
      <section id="hero" style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"88px 32px 60px", position:"relative", zIndex:2 }}>
        <div className="hgrid" style={{ maxWidth:"1080px", width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"56px", flexWrap:"wrap" }}>
          <div style={{ flex:"1 1 400px", maxWidth:"500px" }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"rgba(212,175,55,0.06)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:"100px", padding:"6px 16px", marginBottom:"32px", animation:"borderGlow 3.5s ease infinite" }}>
              <div style={{ width:"6px", height:"6px", borderRadius:"50%", background:"#D4AF37", boxShadow:"0 0 9px #D4AF37", animation:"badgePulse 2.2s ease infinite" }} />
              <span style={{ color:"rgba(212,175,55,0.85)", fontSize:"11px", letterSpacing:"0.13em", textTransform:"uppercase", fontWeight:600 }}>Slack-Native · Now in Beta</span>
            </div>
            <h1 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(42px,5.5vw,70px)", fontWeight:800, letterSpacing:"-0.032em", lineHeight:1.07, marginBottom:"20px", animation:"textGlow 4.5s ease infinite" }}>
              Approvals that{" "}
              <span style={{ background:"linear-gradient(135deg,#D4AF37,#fff0a0 45%,#C49A20)", backgroundSize:"280% 280%", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", animation:"shimmerGold 4s ease infinite" }}>actually work</span>
              {" "}in Slack
            </h1>
            <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(16px,1.9vw,22px)", fontWeight:400, color:"rgba(255,255,255,0.22)", lineHeight:1.35, marginBottom:"22px", minHeight:"32px" }}>
            ...
            <div style={{ display:"flex", gap:"12px", alignItems:"center", flexWrap:"wrap", marginBottom:"18px" }}>
              <MagBtn gold onClick={() => go("cta")} style={{ padding:"15px 32px", borderRadius:"100px", fontSize:"16px", fontWeight:700, color:"#0a0a0a", fontFamily:"'Syne',sans-serif", letterSpacing:"0.04em", boxShadow:"0 8px 30px rgba(212,175,55,0.4)" }}>Get Early Access →</MagBtn>
              <MagBtn outline onClick={() => go("how")} style={{ padding:"15px 28px", borderRadius:"100px", fontSize:"14px", color:"rgba(255,255,255,0.45)", fontFamily:"'DM Sans',sans-serif" }}>See how it works ↓</MagBtn>
            </div>
            <div style={{ color:"rgba(255,255,255,0.15)", fontSize:"11px", letterSpacing:"0.1em", textTransform:"uppercase" }}>Free 14-day trial · No credit card required</div>
          </div>

          <div className="hrw" style={{ flex:"1 1 340px", display:"flex", justifyContent:"center", alignItems:"center", position:"relative", animation:"heroFloat 5.5s ease-in-out infinite" }}>
            {[460,384,308].map((s,i)=>(
              <div key={i} style={{ position:"absolute", top:"50%", left:"50%", width:s, height:s, borderRadius:"50%", transform:"translate(-50%,-50%)",
            ...
            <div style={{ position:"absolute", inset:"-30px", borderRadius:"50%", background:"radial-gradient(circle,rgba(212,175,55,0.07) 0%,transparent 65%)", pointerEvents:"none" }} />
            <SlackCard />
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:"0 32px 80px", position:"relative", zIndex:2 }}>
        <SR>
          <div style={{ maxWidth:"820px", margin:"0 auto" }}>
            <div className="sgrid" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"1px", background:"rgba(212,175,55,0.08)", borderRadius:"22px", overflow:"hidden", border:"1px solid rgba(212,175,55,0.1)" }}>
              {[
      ...
      <div style={{ maxWidth:"300px", margin:"0 auto 80px", height:"1px", background:"linear-gradient(90deg,transparent,rgba(212,175,55,0.3),transparent)", animation:"divShimmer 3s ease infinite" }} />
      ...
      <section id="how" style={{ padding:"0 32px 100px", position:"relative", zIndex:2 }}>
        <div style={{ maxWidth:"940px", margin:"0 auto" }}>
          ...
          <div className="tcol" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"16px" }}>
            {[
              ...
              <SR key={i} delay={i*90}>
                <div className="chov" style={{ background:"linear-gradient(145deg,rgba(14,14,20,0.92),rgba(7,7,12,0.97))", border:"1px solid rgba(255,255,255,0.06)", borderRadius:"20px", padding:"28px 24px", height:"100%" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"18px" }}>
                    <div style={{ width:"46px", height:"46px", borderRadius:"13px", background:"linear-gradient(135deg,#D4AF37,#8B6914)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:"14px", color:"#0a0a0a", boxShadow:"0 7px 22px rgba(212,175,55,0.32)", flexShrink:0 }}>{s.n}</div>
          ...
          <div className="tcol" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"16px" }}>
            {features.map((f,i)=>(
              <SR key={i} delay={f.delay}>
                <div className="chov" style={{ background:"linear-gradient(145deg,rgba(14,14,20,0.92),rgba(7,7,12,0.97))", border:"1px solid rgba(255,255,255,0.06)", borderRadius:"20px", padding:"26px 22px", height:"100%" }}>
      ...
      <section id="pricing" style={{ padding:"0 32px 100px", position:"relative", zIndex:2 }}>
        <div style={{ maxWidth:"940px", margin:"0 auto" }}>
          <SectionHead eyebrow="Pricing" title="Simple," italic="transparent pricing" sub="14-day free trial on all plans. No credit card required to start." />
          <div className="prow" style={{ display:"flex", gap:"18px", flexWrap:"wrap", justifyContent:"center" }}>
            {pricing.map((p,i)=>(
              <SR key={i} delay={i*80}>
                ...
                >
                  ...
                  <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"40px", fontWeight:800, marginBottom:"4px" }}>
                  ...
                  <MagBtn gold={p.featured} outline={!p.featured} onClick={() => { window.location.href = p.href; }}
                    style={{ width:"100%", padding:"13px", borderRadius:"13px", fontSize:"14px", fontWeight:700, fontFamily:"'Syne',sans-serif", letterSpacing:"0.04em",
          ...
          <SR delay={200}>
            <div style={{ marginTop:"24px", textAlign:"center", background:"rgba(212,175,55,0.04)", border:"1px solid rgba(212,175,55,0.14)", borderRadius:"14px", padding:"16px 24px", animation:"borderGlow 4.5s ease infinite", maxWidth:"500px", margin:"24px auto 0" }}>
              <span style={{ color:"rgba(255,255,255,0.45)", fontSize:"13px" }}>
                Save 32% with annual billing — <span style={{ color:"#D4AF37", fontWeight:600 }}>$399/year</span>
            ...
            {testimonials.map((t,i)=>(
              <SR key={i} delay={i*80}>
                <div className="chov" style={{ background:"linear-gradient(145deg,rgba(14,14,20,0.92),rgba(7,7,12,0.97))", border:"1px solid rgba(255,255,255,0.06)", borderRadius:"20px", padding:"26px 22px", height:"100%" }}>
                  ...
                  <p style={{ color:"rgba(255,255,255,0.65)", fontSize:"14px", lineHeight:1.78, marginBottom:"18px", fontStyle:"italic" }}>&ldquo;{t.q}&rdquo;</p>
                  <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
      ...
      <section id="cta" style={{ padding:"0 32px 110px", position:"relative", zIndex:2 }}>
        <SR>
          <div style={{ maxWidth:"740px", margin:"0 auto", textAlign:"center" }}>
            <div style={{ background:"linear-gradient(150deg,rgba(20,15,6,0.98),rgba(10,8,3,0.99))", border:"1px solid rgba(212,175,55,0.22)", borderRadius:"30px", padding:"68px 52px", boxShadow:"0 50px 120px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.03)", position:"relative", overflow:"hidden", animation:"borderGlow 4s ease infinite" }}>
              ...
              <div style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"rgba(34,197,94,0.07)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:"100px", padding:"6px 16px", marginBottom:"26px" }}>
              ...
              <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(28px,3.8vw,48px)", fontWeight:800, letterSpacing:"-0.025em", lineHeight:1.1, marginBottom:"16px", animation:"textGlow 4.5s ease infinite" }}>
                Stop losing approvals<br />
                <span style={{ background:"linear-gradient(135deg,#D4AF37,#fff0a0 45%,#C49A20)", backgroundSize:"280% 280%", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", animation:"shimmerGold 4s ease infinite" }}>in the Slack noise</span>
              </h2>
              <p style={{ color:"rgba(255,255,255,0.33)", fontSize:"16px", lineHeight:1.85, fontWeight:300, maxWidth:"480px", margin:"0 auto 38px" }}>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ borderTop:"1px solid rgba(255,255,255,0.04)", padding:"36px 48px", position:"relative", zIndex:2, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"18px" }}>
